...
mixin template Semantic(T) if(is(T==Expression)){
	...
	Dependent!Expression matchCallHelper(Scope sc, const ref Location loc, Type this_, Expression[] args, ref MatchContext context)in{
		...
		auto rd=type.matchCallHelper(sc, loc, this_, args, context);
...
class TemplateInstanceDecl: Declaration{
	...
	private void initializeIFTI(){
		...
		if(!fd){ // TODO: this is probably not required (the test suite also not cover it)
...
class Symbol: Expression{ // semantic node
	...
	void performAccessCheck() in{assert(meaning && scope_);}body{
		...
		if(auto decl = scope_.getDeclaration()){
			mixin(IsDeclAccessible!q{auto b; decl, meaning});
			if(!b){
	...
	override bool templateParameterEquals(Expression rhs){
		if(auto sym = rhs.isSymbol()) return meaning is sym.meaning;
...
mixin template Semantic(T) if(is(T==FieldExp)){
	...
	override void matchError(Scope sc, Location loc, Type th_, Expression[] args){
		...
		Type this_ = null;
		if(auto tt=e1.extractThis()) this_=tt.type;
		if(tuple) super.matchError(sc,loc,this_,args);
...
mixin template Semantic(T) if(is(T==Type)){
	...
	final Type applyScopeSTC(Scope sc){
		...
		if(auto fd=fl.isFunctionDecl()){
			type = type.applySTC(fl.stc&(STCconst|STCimmutable|STCshared|STCinout));
		}
		return type;
	}
...
mixin template Semantic(T) if(is(T==Declaration)){
	...
	final Dependent!bool isDeclAccessible(Declaration decl)in{
		...
		if(auto fd=decl.isFunctionDecl()){
			if(fd.isConstructor())
				return isDeclAccessible(fd.scope_.getDeclaration());
		}
		...
		for(Declaration dl=this; dl !is null && dl !is enc; dl=dl.scope_.getDeclaration()){
			if(auto raggr=dl.isReferenceAggregateDecl()){
				mixin(AggregateParentsInOrderTraversal!q{
					mixin(IsDeclAccessible!q{auto b; parent, decl});
					if(b) return true.independent;
				});
			}
			if(this is enc) return (!isAggregateDecl()).independent;
		...
		if(auto tp = type.isTypeTuple()){
			...
			if(!tc.vds){
				...
				foreach(i, ref x;tc.vds){
					...
					auto ini = init?et.index(sc,init.loc,i):null;
		...
		if(isClassDecl()){
			...
			if(parents.length){
				if(parents[0].needRetry||parents[0].sstate==SemState.error)
					return Dependee(parents[0], scope_).dependent!void;
				if(rparents[0].sstate == SemState.error)
					return Dependee(rparents[0], scope_).dependent!void;
				...
				if(auto cdecl=parentc.isClassDecl()){
					...
					if(parent.needRetry||parent.sstate==SemState.error)
						return Dependee(parent, parent.scope_).dependent!void;
					if(!vtbl.length){
		...
		foreach(ref x; parents[knownParents..n]){
			...
			if(ty){
				void check(Type ty){
					...
					if(!agg||!agg.decl.isReferenceAggregateDecl()){
						sc.error("base specifier must name a class or interface", x.loc);
						mixin(SetErr!q{x});
					}
				}
	...
	Lvtbl:
		...
		foreach(x; vtbl.vtbl){
			if(x.state == VtblState.needsOverride){
				mixin(Lookup!q{auto ovsc; asc, x.fun.name, null});
				assert(!!ovsc);
		...
		if(num>1){
			...
			if(!compatd.value){
				...
				if(isConstructor){
					...
					sc.error(format("incompatible 'this' pointer is %s, but %s is required",hstcstr, fstcstr), loc);
				}
